...
다섯 질문을 실제 수업에 녹이는 법(수업 설계·실천 순서): ①무엇이 중요할까—도구보다 배움 먼저, "이 차시에 무슨 역량을 기를까"부터 정하고 AI·디지털은 수단으로("AI 쓰니 재밌다"가 아니라 "AI로 사례 비교해 개념을 스스로 발견"). ②무엇부터—작게 한 지점부터(형성평가 카훗, 의견모으기 패들렛 등 부담 적은 활동), 전면 도입 금물. ③어떻게 설계—성취기준→학습목표→평가→도구 순(백워드), 도구가 목표 흔들지 않게(예: 생성AI 초안을 학생이 비판·수정하는 과정을 평가). ④어떻게 실천—교사주도성·설명가능한 수업(왜 이 방식인지 교사가 설명 가능, AI 결과는 학생이 검증; "이게 맞는지 어떻게 알까?" 질문으로 AI 리터러시·윤리 연결). ⑤어떻게 지속—잘 된 수업을 지도안·워크시트로 기록·공유해 학교문화로, 이 기록이 보고서 우수사례·정량 근거가 됨.
