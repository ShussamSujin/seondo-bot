...
- 정량 데이터: 사전·사후 조사는 필수가 아님! 정량화 방법은 다양 — 교원학습공동체 참여 교원 수, 연수 참여 횟수·인원, 적용 학급·학생 수, 공개수업 횟수 같은 실적 수치도 정량 데이터. 사전·사후 비교를 원하면 KERIS 공식 설문 문항을 사전·사후 형식으로 변환 재활용, RISS·KERIS 역량 검사도구 활용, 구글 설문+QR 배포, 같은 문항으로 사전/사후, 이름 미수집, 표본 수(n) 표기, "디지털 도구 덕분에" 같은 단정적 해석 지양.
